                                ...
                                @foreach($rack_shocks_array as $rack_shocks_size)
                                    ...
                                    <div class="card">
                                        <div class="card-header">
                                            <a href="javascript:void(0);" class="card-title" data-toggle="collapse" data-target="#1" onclick="collapaseShow({{ $rack_shocks_size['size_id'] }})" aria-expanded="true">
                                        ...
                                        <div id="m" class="collapse shocks_collapse_{{ $rack_shocks_size['size_id'] }}" data-parent="#1">
                                            <div class="card-body">
                                                <div class="frame-wrap">
                                                    <div class="custom-control custom-checkbox">
                                                        <input type="checkbox" class="custom-control-input" id="{{ $rack_shocks_size['size_id'] }}">
                                                        <label class="custom-control-label" onclick="selectAllSocks({{ $rack_shocks_size['size_id'] }})" for="{{ $rack_shocks_size['size_id'] }}">Select All {{ $rack_shocks_size['name'] }} Socks</label>
                                                    </div>

                                                    <hr>

                                                    <div class="demo">
                                                        @foreach($rack_shocks_array[$rack_shocks_size['size_id']]['shocks'] as $size_single_shocks)
                                                            <div class="custom-control custom-checkbox single_checkbox">
                                                                <input type="checkbox" onclick="countTotalSoldoutSocks('{{ $size_single_shocks['shocks_code'] }}')" class="select_items custom-control-input rack_single_shocks_{{  $size_single_shocks['shocks_size_id'] }}" value="{{ $size_single_shocks['shocks_code'] }}" id="{{ $size_single_shocks['shocks_code'] }}">
                                                                <label  class="custom-control-label" for="{{ $size_single_shocks['shocks_code'] }}">{{ $size_single_shocks['print_shocks_code'] }} ({{  $size_single_shocks['brand_name'] }} - {{  $size_single_shocks['brand_size_name'] }}) - {{ date('jS F,Y', strtotime($size_single_shocks['sold_mark_date_time'])) }}</label>
                                                            </div>
                                                        @endforeach                                                        
                                                    </div>
                                                </div>
                        ...
                        success: (data) => {
                            ...
                            if(data.status === 200){
                                cuteAlert({
                                    ...
                                    //window.location.replace("http://localhost:8080/pegasus/rack-bill/voucher-stream/chaina-pavilion-vls1-6.pdf");
